...
public class UserUtilsServiceImpl extends BaseUserService implements IUserUtilsService {
    ...
    @Override
    public ResponseResult<Long> getRegisterCount() {
        long count = userDao.count();
        return new ResponseResult(HttpStatus.OK, "获取用户总数成功.",count);
    }
    ...
    public ResponseResult checkEmailCode(String email, String emailCode, String captchaCode) {
        ...
        return new ResponseResult(HttpStatus.OK, "验证码正确");
    }


    @Override
    public ResponseResult<Map<String, Object>> getPcLoginQrCodeInfo() {
        //尝试取出上一次的loginId
...
//        }
        ...
        // 返回结果
        return new ResponseResult(HttpStatus.OK, "获取成功.",result);
    }



    @Override
    public ResponseResult<UserEntity> parseToken() {
        UserEntity sobUser = checkSobUser();
        if (sobUser == null) {
            return new ResponseResult(HttpStatus.BAD_REQUEST, "用户未登录.");
        }
        return new ResponseResult(HttpStatus.OK, "获取用户成功.",sobUser);
    }


    @Override
    public ResponseResult checkEmail(String email) {
        UserEntity user = userDao.findOneByEmail(email);
        return user == null ?
                new ResponseResult(HttpStatus.OK, "该邮箱未注册.")
                : new ResponseResult(HttpStatus.BAD_REQUEST, "该邮箱已经注册.");
    }
    ...
    public ResponseResult checkUserName(String userName) {
        ...
        return user == null ?
                new ResponseResult(HttpStatus.OK, "该用户名未注册.")
                :new ResponseResult(HttpStatus.BAD_REQUEST, "该用户名已经存在.");
    }
    ...
    public ResponseResult sendEmail(String type, HttpServletRequest request, String address, String captchaCode) {
        ...
        if ("register".equals(type) || "update".equals(type)) {
            UserEntity userByEmail = userDao.findOneByEmail(address);
            if (userByEmail != null) {
                return new ResponseResult(HttpStatus.BAD_REQUEST, "该邮箱已注册");
            }
        } else if ("forget".equals(type)) {
            UserEntity userByEmail = userDao.findOneByEmail(address);
            if (userByEmail == null) {
                return new ResponseResult(HttpStatus.BAD_REQUEST, "该邮箱未注册");
            }
        }
        ...
        if (ipSendTime > 10) {
            return new ResponseResult(HttpStatus.BAD_REQUEST, "您发送验证码也太频繁了吧！");
        }
        ...
        try {
            ...
            return new ResponseResult(HttpStatus.BAD_REQUEST, "验证码发送失败，请稍后重试.");
        }
        //4、做记录
        //发送记录，code
        //
        ...
        return new ResponseResult(HttpStatus.OK, "验证码发送成功");
    }
}
