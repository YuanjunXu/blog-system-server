...
import blog.system.server.dao.RefreshTokenEntityRepository;
...
import blog.system.server.service.IBaseUserService;
...
public class BaseUserService extends BaseService implements IBaseUserService {
    ...
    @Autowired
    protected RefreshTokenEntityRepository refreshTokenDao;
    ...
    @Autowired
    protected UserEntityRepository userDao;
     ...
     */
    @Override
    public UserEntity checkSobUser() {
        //拿到token_key
        ...
        UserEntity sobUser = parseByTokenKey(tokenKey);
        //token中要解析此请求是什么端的
        ...
        if (sobUser == null) {
            ...
            try {
                ...
                UserEntity userFromDb = userDao.findOneById(userId);
     ...
     * @return token_key
     */
    protected String createToken(HttpServletResponse response, UserEntity userFromDb, String from) {
        String oldTokenKey = CookieUtils.getCookie(getRequest(), Constants.User.COOKIE_TOKE_KEY);
        //不能干掉了
        RefreshTokenEntity oldRefreshToken = refreshTokenDao.findOneByUserId(userFromDb.getId());
        ...
        //如果有的话就更新
        //如果没有就新创建
        RefreshTokenEntity refreshToken = refreshTokenDao.findOneByUserId(userFromDb.getId());
        if (refreshToken == null) {
    ...
    protected UserEntity parseByTokenKey(String tokeKey) {
        String token = (String) redisUtils.get(Constants.User.KEY_TOKEN + tokeKey);
